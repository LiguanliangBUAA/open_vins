...
void ROS2Visualizer::visualize_odometry(double timestamp) {
  ...
  if (pub_odomimu->get_subscription_count() != 0) {
    ...
    odomIinM.header.frame_id = "drone0/ov_odom";
    ...
    odomIinM.child_frame_id = "drone0/local_est/imu";
  ...
  trans.child_frame_id = "drone0/local_est/imu";
  ...
  for (const auto &calib : state->_calib_IMUtoCAM) {
    ...
    trans_calib.header.frame_id = "drone0/local_est/imu";
    trans_calib.child_frame_id = "drone0/local_est/cam0" + std::to_string(calib.first);
...
void ROS2Visualizer::publish_images() {
  ...
  header.frame_id = "drone0/local_est/cam0";
...
void ROS2Visualizer::publish_groundtruth() {
  ...
  if (!gt_aligned) {
    ...
    Eigen::Vector3d ypr = R_full.eulerAngles(2, 1, 0) * 180.0 / M_PI;
    PRINT_INFO("[ALIGN-DBG] R_full ypr = %.1f, %.1f, %.1f deg\n", ypr(0), ypr(1), ypr(2));
...
void ROS2Visualizer::publish_loopclosure_information() {
  ...
  if (pub_loop_pose->get_subscription_count() != 0 || pub_loop_extrinsic->get_subscription_count() != 0 ||
    ...
    odometry_pose.header.frame_id = "drone0/ov_odom";
    ...
    odometry_calib.header.frame_id = "drone0/local_est/imu";
    ...
    cameraparams.header.frame_id = "drone0/local_est/cam0";
  ...
  if (pub_loop_point->get_subscription_count() != 0) {
    ...
    point_cloud.header.frame_id = "drone0/ov_odom";
